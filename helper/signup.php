...
<a class="btn btn-large btn-block btn-primary" href="/wgp/">Access SimpleCloud</a>
<button class="btn btn-large btn-block btn-primary" type="button">Settings</button>
<?php
}else{
?>
...
/*	$('#signup-user').focusout(function(eventObject){
		...
		$('#signup-form').find('input').each(function(index){
			if($(this).attr('type')!='submit'){
				...
				if(value.length>len){
					...
					$(this).tooltip('show');
					flag = 1;
				}else if($(this).is('#signup-user') && !usernameIsValid(value)){
					$(this).parent().addClass("warning");
					$(this).tooltip({title:'Username not available',trigger:"focus",placement:"top"});
					$(this).tooltip('show');
						flag = 1;
				}else{
					$('#'+id+'_comment').text('');
			...
			$.ajax({
					...
					success: function(data){
						...
						if(data['error']===undefined){
							$("#signup-area").fadeOut("fast",function(){$("#signup-area").load('/test/web/helper/signup-success.php',$("#signup-area").fadeIn('fast'));});
							
						}else{
							if(data['error']['username']!=undefined && data['error']['username']['invalid']!=undefined){
								$('#signup-user').next('.help-inline').html('<i class="icon-remove-circle"></i>');
								$('#signup-user').tooltip({title:'Invalid username - User name can only contain alphanumeric characters and .-_',trigger:"hover",placement:"left",delay:{show: 500, hide: 100}});
								//$('#signup-user').next('.help-inline').tooltip('show');
							}
							if(data['error']['username']!=undefined && data['error']['username']['unavailable']!=undefined){
								$('#signup-user').next('.help-inline').html('<i class="icon-remove-circle"></i>');
								$('#signup-user').tooltip({title:'Username unavailable - Please pick up another one.',trigger:"hover",placement:"left",delay:{show: 500, hide: 100}});
								//$('#signup-user').next('.help-inline').tooltip('show');
							}
							...
							if(data['error']['username']!=undefined && data['error']['username']['empty']!=undefined){
								$('#signup-user').next('.help-inline').html('<i class="icon-remove-circle"></i>');
								$('#signup-user').tooltip({title:"This field cannot be empty.",trigger:"hover",placement:"left",delay:{show: 500, hide: 100}});
								//$('#signup-user').next('.help-inline').tooltip('show');
							}	
						}
	...
	function usernameIsValid(username) {
		return /^[0-9a-z_.-]+$/.test(username);
				...
				<h3>Sign up for a 2Gb free account !</h3>  
    ...
    ================================================== -->					
					...
					<div class="control-group">
						<input type="text" name="user" id="signup-user" value="" maxlength="20" style="text-transform: lowercase" placeholder="username"/>					
						<span class="help-inline"></span>			
					</div>	
					<div class="control-group">
